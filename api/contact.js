export default async function handler(req, res) {
  ...
  try {
    const { name, email, subject, project_category, budget_scale, message } = req.body;

    const apiKey = process.env.RESEND_API_KEY;
    if (!apiKey) {
      return res.status(500).json({ success: false, message: "Resend API Key is missing on the server." });
    }
...
<html>
  ...
  <body style="margin: 0; padding: 0; background-color: #fafafa; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;">
    ...
    if (resendResponse.ok) {
      ...
      return res.status(resendResponse.status).json({ success: false, message: result.message || "Resend submission failed." });
    }
  } catch (error) {
